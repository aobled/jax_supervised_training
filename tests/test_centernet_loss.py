...
def test_size_loss_positive_and_decreasing_towards_gt():
    """Depuis le changement de perte 2026-07-18 (echelle logarithmique, voir
    compute_size_regression_loss/loss_functions.py, discussion post-Epic 8) : pred_size
    est desormais interprete comme log(taille), pas la taille brute elle-meme -
    "proche"/"loin" de la cible doivent donc etre exprimes en echelle log ici, sinon le
    test ne teste plus le bon signal (une prediction en pixels bruts proche de gt_size
    en LINEAIRE serait en realite tres loin de log(gt_size), faussant l'assertion)."""
    _, gt_size = _make_gt()
    log_gt_size = jnp.log(jnp.where(gt_size > 0.0, gt_size, 1.0))

    pred_far = jnp.zeros_like(gt_size)  # log-taille predite = 0.0, loin de log(12)=2.48/log(20)=3.0
    pred_close = log_gt_size + 0.01  # log-taille predite tres proche de la cible, en echelle log
...
def test_compute_centernet_loss_decreases_and_uses_dict_contract():
    ...
    # SIZE_KEY des outputs en echelle log depuis 2026-07-18 (voir test_size_loss_positive_
    # and_decreasing_towards_gt) - "close" doit etre proche de log(gt_size), pas de gt_size.
    log_gt_size = jnp.log(jnp.where(gt_size > 0.0, gt_size, 1.0))
