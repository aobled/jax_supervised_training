...
def test_out_of_bounds_box():
    ...
    out_constant = os.path.join(script_dir, "test_pixel_parity_oob_constant.png")
    out_nearest = os.path.join(script_dir, "test_pixel_parity_oob_nearest.png")
    cv2.imwrite(out_constant, arr_constant.astype(np.uint8))
    cv2.imwrite(out_nearest, arr_nearest.astype(np.uint8))
    print(f"Images sauvegardees : {out_constant} / {out_nearest}")
