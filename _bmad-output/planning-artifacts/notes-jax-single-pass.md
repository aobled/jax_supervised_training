...
**Reconfirmation (2026-07-15, même question reposée par Aymeric)** : ratio quantifié — 1920×1080 vs 224×224 ≈ **41× plus de pixels** si le dataset devait être stocké en full-HD. Confirmé non nécessaire : `fighterjet_detection_dataset_tools.py` (l'outil **actuel**) fait déjà "résolution source quelconque → 224×224 stocké, coordonnées de boîte rescalées proportionnellement" (lignes 120-123) — ce n'est pas un nouveau comportement introduit par JAX Single-Pass, juste la continuation du pattern existant dans `_v2.py`. Nuance mineure notée sans y donner suite pour l'instant : si les images sources utilisées pour construire le dataset d'entraînement ne sont pas elles-mêmes de résolution proche du full-HD, le ratio de sous-échantillonnage vu à l'entraînement (source→224) pourrait différer légèrement de celui vu à l'inférence (1920×1080→224, ratio ~8.6×) — distinct du risque de parité de méthode de resize déjà noté, probablement pas significatif si les sources d'entraînement sont déjà variées en résolution.
...
- **Gap comblé (2026-07-15, architecture spine)** : la spine parente (`ARCHITECTURE-SPINE.md` Epic 1-3, AD-8) nommait déjà `tools/audit_dataset_detection.py` et `tools/boxes_process_manual_tkinter.py` comme consommateurs de `decode_segmentation_and_detect(_batch)` invisibles à une lecture superficielle — vérifié, ils le sont toujours (`grep` confirmé). Absents du triage initial ci-dessus, ajoutés ici. Restent sur l'ancien pipeline segmentation tant que non explicitement migrés (pas de casse tant qu'aucun symbole n'est retiré de `inference_utils.py`, juste un périmètre de migration à ne pas oublier le moment venu).
