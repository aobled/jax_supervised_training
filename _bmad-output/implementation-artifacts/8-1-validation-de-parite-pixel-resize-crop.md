...
- [x] Task 1: Script autonome `test_pixel_parity.py` (racine, même convention que les tests des Stories 7.1/7.5) — pas de framework de test, exécutable directement
- [x] Task 2: Test RESIZE — sur 2-3 images réelles 1920×1080 **grayscale** (mode PIL `'L'`, cohérent avec la préparation des chunks `JAX_DETECTOR`, `fighterjet_detection_dataset_tools.py:98` — pas des images RGB arbitraires, la distribution testée doit correspondre à celle vue à l'entraînement) : (AC: 1)
...
- Nouveau fichier `test_pixel_parity.py` à la racine (script de validation autonome, pas un module consommé ailleurs).
...
- [Source: `fighterjet_detection_dataset_tools.py:104`] — `Image.resize(target_size, Image.Resampling.LANCZOS)`, méthode de référence pour la préparation des chunks détection
...
`python3 test_pixel_parity.py` — sortie complète : RESIZE testé sur 3 images réelles 1920×1080 (`test_media/testvid0{1,2,3}.png`, fournies par l'utilisateur) × 4 combinaisons méthode/antialias ; CROP testé sur les 21 boîtes réelles disponibles (`test_media/testvid0{1,2,3}_*.json`, même format que Story 7.1) pour les cas entier et flottant ; cas hors-cadre construit délibérément et vérifié visuellement (`test_pixel_parity_oob_constant.png`/`_nearest.png`).
...
- `test_pixel_parity.py` (nouveau, racine) — script de validation autonome complet (Tasks 2-5).
- `test_pixel_parity_oob_constant.png`, `test_pixel_parity_oob_nearest.png` (nouveaux, racine) — preuve visuelle Task 5.
