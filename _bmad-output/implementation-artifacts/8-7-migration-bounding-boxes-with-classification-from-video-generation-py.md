...
- `python3 archive/capture_baseline_video_8_7.py` (`JAX_PLATFORMS=cpu`) — ancien pipeline (UNet+findContours) sur 64 premières frames réelles de `testvid.mp4` : 431 détections, 1,95 fps (CPU, warmup détection+classification symétrique), sauvegardé dans `archive/baseline_video_8_7.json`.
- `python3 archive/capture_migrated_video_8_7.py` (`JAX_PLATFORMS=cpu`) — nouveau pipeline (Single-Pass, Story 8.6) sur le même extrait : 448 détections, 0,70 fps (CPU), sauvegardé dans `archive/migrated_video_8_7.json`.
- `python3 archive/diff_baseline_vs_migrated_video_8_7.py` — comparaison : ordre de grandeur comparable (431 vs 448, +4%), aucune frame avec moins de détections que la baseline (0 en moins, 16 en plus, 48 identiques en compte), classes prédites globalement cohérentes entre les deux chemins (mêmes classes dominantes, distribution différente — attendu, détecteur différent).
...
- **Task 1** : baseline capturée via un script dédié (`archive/capture_baseline_video_8_7.py`) réutilisant les fonctions réelles de l'ancien chemin (`decode_segmentation_and_detect_batch`, `predict_crops_batch`) sur un extrait fixe de 64 frames (pas la vidéo complète, 599 frames — un "extrait", cohérent avec le texte de la Task) de `testvid.mp4` (= `test_media/testvid.mp4`, fourni par l'utilisateur).
...
- `archive/capture_baseline_video_8_7.py` (nouveau, racine) — capture de la baseline avant migration (Task 1)
- `archive/capture_migrated_video_8_7.py` (nouveau, racine) — capture des résultats après migration (Task 6/7)
- `archive/diff_baseline_vs_migrated_video_8_7.py` (nouveau, racine) — comparaison quantitative (Task 6)
- `archive/baseline_video_8_7.json`, `archive/migrated_video_8_7.json` (nouveaux, racine) — données brutes capturées, réutilisables pour la Story 8.9
