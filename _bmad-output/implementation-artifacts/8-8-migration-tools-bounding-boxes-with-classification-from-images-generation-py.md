...
- `python3 capture_baseline_images_8_8.py` (`JAX_PLATFORMS=cpu`) — ancien pipeline (UNet+NMS) sur 3 images réelles (`test_media/testvid0{1,2,3}.png`, copiées dans un répertoire scratch isolé) : 19 détections, sauvegardé dans `baseline_images_8_8.json`.
- `python3 capture_migrated_images_8_8.py` (`JAX_PLATFORMS=cpu`) — nouveau pipeline (Single-Pass, Story 8.6) sur le même jeu : 20 détections, sauvegardé dans `migrated_images_8_8.json`.
- `python3 diff_baseline_vs_migrated_images_8_8.py` — comparaison par image : `testvid01.png` 7/7, `testvid02.png` 7/7, `testvid03.png` 5/6 — ordre de grandeur cohérent (proche des 7 boîtes annotées réelles par image, Story 8.1), classes prédites globalement cohérentes entre les deux chemins.
...
- `capture_baseline_images_8_8.py` (nouveau, racine) — capture de la baseline avant migration (Task 1)
- `capture_migrated_images_8_8.py` (nouveau, racine) — capture des résultats après migration (Task 9)
- `diff_baseline_vs_migrated_images_8_8.py` (nouveau, racine) — comparaison quantitative (Task 9)
- `baseline_images_8_8.json`, `migrated_images_8_8.json` (nouveaux, racine) — données brutes capturées, réutilisables pour la Story 8.9
