...
- Réutilisation directe de `baseline_video_8_7.json`/`migrated_video_8_7.json` (Story 8.7, 64 frames réelles de ce même fichier, ancien pipeline vs nouveau pipeline) — pas une nouvelle capture, ces données sont déjà la preuve par exécution demandée par cette story sur exactement le cas identifié.
...
- Réutilisation de `baseline_video_8_7.json`/`migrated_video_8_7.json` (Story 8.7, aucune nouvelle capture nécessaire)
