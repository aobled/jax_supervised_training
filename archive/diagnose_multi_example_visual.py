...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "diagnose_multi_example_visual.png")
...
for images, targets in val_ds.take(6):
    ...
    # Sortie brute du reseau desormais en echelle log (2026-07-18, voir
    # compute_size_regression_loss) - exp() pour retrouver des pixels comparables a
    # gt_size (reste en pixels bruts, cible inchangee) et dessinables tels quels.
    pred_size = np.exp(np.asarray(outputs[SIZE_KEY], dtype=np.float32))
