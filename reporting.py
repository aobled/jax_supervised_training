...
class ViTAttentionVisualizer:
    """
    Visualise les Attention Maps des Vision Transformers
    Montre quelles parties de l'image le modèle regarde pour décider
    """
    
    def __init__(self, state, model, class_names):
        """
        Args:
            state: TrainState du modèle
            model: Instance du modèle ViT
    ...
    def visualize_attention(self, test_images, test_labels, epoch, save_path=None, num_samples=8):
        """
        Génère une visualisation des attention maps sur des images test
        Layout: 4 lignes × 4 colonnes (2 paires d'images par ligne)
        
        Args:
            test_images: Batch d'images test (B, H, W, C)
            test_labels: Labels correspondants (B,)
            epoch: Numéro d'epoch actuel
            save_path: Chemin de sauvegarde (défaut: attention_map_epoch_XX.png)
            num_samples: Nombre d'images à visualiser (défaut: 8)
        """
        if save_path is None:
            save_path = f"attention_map_epoch_{epoch:03d}.png"
        ...
        # Créer figure avec grille : 4 lignes × 4 colonnes
        # Chaque ligne : [Image1, AttentionMap1, Image2, AttentionMap2]
        ...
        for idx in range(num_samples):
            ...
            # Calculer position dans la grille 4×4
            # idx 0,1 → ligne 0, colonnes 0-1 et 2-3
            # idx 2,3 → ligne 1, colonnes 0-1 et 2-3
            # etc.
            ax_img_idx = idx * 2      # Image: colonnes 0,2,4... → 0,2,4,6,8,10,12,14
            ax_att_idx = idx * 2 + 1  # Attention: colonnes 1,3,5... → 1,3,5,7,9,11,13,15
            
            # Prédiction
            vars = {"params": self.state.params}
            if self.state.batch_stats is not None and self.state.batch_stats != {}:
                vars["batch_stats"] = self.state.batch_stats
            
            # Forward pass pour obtenir prédiction
            img_batch = jnp.expand_dims(img, 0)
            logits = self.state.apply_fn(vars, img_batch, training=False)
            pred_label = int(jnp.argmax(logits[0]))
            pred_proba = float(jax.nn.softmax(logits[0])[pred_label])
            
            # Labels
            true_class = self.class_names[true_label] if true_label < len(self.class_names) else f"Class {true_label}"
            pred_class = self.class_names[pred_label] if pred_label < len(self.class_names) else f"Class {pred_label}"
            
            # Couleur selon correct/incorrect
            title_color = 'green' if pred_label == true_label else 'red'
            
            # Plot 1 : Image originale (juste True: classe)
            ax = axes[ax_img_idx]
            if img.shape[-1] == 1:  # Grayscale
                ax.imshow(img.squeeze(), cmap='gray')
            else:  # RGB
                ax.imshow(img)
            ax.set_title(f"True: {true_class}", fontsize=11, pad=3)
            ax.axis('off')
            
            # Plot 2 : Attention map avec prédiction en titre
            ax = axes[ax_att_idx]
            try:
                # Utiliser gradient comme proxy d'attention
                def get_attention_proxy(params, batch_stats, img_batch_input):
                    vars_dict = {'params': params}
                    if batch_stats is not None and batch_stats != {}:
                        vars_dict['batch_stats'] = batch_stats
                    
                    def pred_fn(img_in):
                        logits = self.model.apply(vars_dict, img_in, training=False)
                        return logits[0, pred_label]
                    
                    grad = jax.grad(pred_fn)(img_batch_input)
                    return jnp.abs(grad[0]).mean(axis=-1)
                
                attention_proxy = get_attention_proxy(self.state.params, self.state.batch_stats, img_batch)
                ...
                # Afficher heatmap superposée
                ax.imshow(img.squeeze() if img.shape[-1] == 1 else img, cmap='gray' if img.shape[-1] == 1 else None)
                im = ax.imshow(attention_proxy, alpha=0.5, cmap='hot')
                
                # Titre avec prédiction et confiance (en vert/rouge)
                ax.set_title(f"Pred: {pred_class} ({pred_proba:.0%})", 
                           fontsize=11, color=title_color, fontweight='bold', pad=3)
                ax.axis('off')
        ...
        plt.suptitle(f"Attention Maps - Epoch {epoch}", fontsize=16, fontweight='bold', y=0.995)
        plt.tight_layout(rect=[0, 0, 1, 0.99])  # Laisser de la place pour le titre
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
        
        print(f"🔍 Attention maps sauvegardées: {save_path} (8 échantillons)")
